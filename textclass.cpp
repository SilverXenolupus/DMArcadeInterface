	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../DMArcadeInterface/data/fontrules.txt", L"../DMArcadeInterface/data/roboto.dds");
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, "^", 20, 30, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "RECTANGLE!!!", 1080, 800, 0.0f, 0.5f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	ID3D11DeviceContext* deviceContext)
{
	...
	drawX = (float)(((m_screenWidth / 2) * -1) + positionX/* - numLetters * 14*/);

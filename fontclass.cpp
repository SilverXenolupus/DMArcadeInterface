...
void FontClass::BuildVertexArray(void* vertices, char* sentence, float drawX, float drawY)
{
	...
	for (i = 0; i<numLetters; i++)
	{
		...
		// If the letter is a space or underscore then just move over ten pixels.
		...
		else
		{
			...
			vertexPtr[index].position = D3DXVECTOR3((drawX + m_Font[letter].size), (drawY - 32), 0.0f);  // Bottom right.
			vertexPtr[index].texture = D3DXVECTOR2(m_Font[letter].right, 1.0f);
			index++;

			vertexPtr[index].position = D3DXVECTOR3(drawX, (drawY - 32), 0.0f);  // Bottom left.
